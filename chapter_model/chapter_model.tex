...
To ensure that $\log_{2}(\mathrm{FC}_{i,j})$ can be calculated for all $i$ and $j$ so that changes in proteome allocation can be shown in Fig. \ref{fig:model-ablate-enz-use}, small $e_{i}$ values with magnitudes below a threshold $\epsilon$ were rounded to the threshold value.
This rounding eliminates cases where $e_{i} = 0$, which would make $\log_{2}(\mathrm{FC}_{i,j})$ undefined.
...
Therefore, if there is a case of enzyme expression switching on, evidenced by $e_{i, \mathrm{original}} = 0$, $e_{i,\mathrm{original}}$ would be replaced with $\epsilon$, and as a consequence, $\log_{2}(\mathrm{FC}_{i,j}) \ll 0$.
Conversely, if there is a case of an enzyme expression switching off, evidenced by $e_{i, j} = 0$, $e_{i,j}$ would be replaced with $\epsilon$, and as a consequence, $\log_{2}(\mathrm{FC}_{i,j}) \gg 0$.
...
\begin{figure}
  ...
  \caption{
    ...
    Fluxes with magnitude less than $\epsilon = $ \SI{1.11d-10}{\mmolgdw} are treated as $\epsilon$ flux, for the purpose of computing fold changes.
