...
where $\mathrm{MW}_{i}$ represents the molecular weight of the enzyme in units of \SI{}{\gram~\milli\mole^{-1}}, and $E_{\mathrm{pool}}$ is a pseudometabolite that represents the proteome pool available for enyzmes.
...
To constrain overall enzyme levels in the model, GECKO defines a pseudo-reaction
...
\begin{itemize}
  ...
  \item $f_{i}$ is the mass fraction of each biomass component (table~\ref{tab:model-biomfrac}), and
...
\begin{figure}
  ...
  \caption{
    Changes in enzyme usage reaction flux in rounds of ablation, in a (\ref{fig:model-rank-glc-highratio-rank}) high $\ratioabl$ condition ($\exchrate{glc}$ = \SI{1.69}{\mmolgdwh}, $\exchrate{amm}$ = \SI{1.05}{\mmolgdwh}), and (\ref{fig:model-rank-glc-lowratio-rank}) a low $\ratioabl$ condition ($\exchrate{glc}$ = \SI{16.89}{\mmolgdwh}).
...
\begin{figure}
  ...
  \caption{
    ...
    The computations were performed for a (\ref{fig:model-rank-glc-highratio-kendall}) high $\ratioabl$ condition ($\exchrate{glc}$ = \SI{1.69}{\mmolgdwh}, $\exchrate{amm}$ = \SI{1.05}{\mmolgdwh}), and  (\ref{fig:model-rank-glc-lowratio-kendall}) a low $\ratioabl$ condition ($\exchrate{glc}$ = \SI{16.89}{\mmolgdwh}).
  }
...
\begin{figure}
  ...
  \caption{
    Changes in enzyme usage reaction flux in rounds of ablation, in a  (\ref{fig:model-rank-pyr-highratio-rank}) high $\ratioabl$ condition ($\exchrate{pyr}$ = \SI{3.73}{\mmolgdwh}, $\exchrate{amm}$ = \SI{0.90}{\mmolgdwh}), and (\ref{fig:model-rank-pyr-lowratio-rank}) a low $\ratioabl$ condition ($\exchrate{pyr}$ = \SI{8.89}{\mmolgdwh}).
    Columns show the biomass component prioritised.
    In each column, rows represent enzyme usage reactions, arranged in descending order of flux.
    Colours identify the reactions, with white indicating reactions that carry zero flux in the parallel case.
  }
...
\begin{figure}
  ...
  \caption{
    ...
    The computations were performed for a (\ref{fig:model-rank-pyr-highratio-kendall}) high $\ratioabl$ condition ($\exchrate{pyr}$ = \SI{3.73}{\mmolgdwh}, $\exchrate{amm}$ = \SI{0.90}{\mmolgdwh}), and a (\ref{fig:model-rank-pyr-lowratio-kendall}) low $\ratioabl$ condition ($\exchrate{pyr}$ = \SI{8.89}{\mmolgdwh}).
  }
...
\begin{figure}
  ...
  \caption{
    ...
    Cosine distances between the between the vector of enzyme usage fluxes when protein is prioritised and the equivalent vector when carbohydrate is prioritised are shown \ref{fig:model-noisy-glc-cosine} for glucose-ammonium conditions and \ref{fig:model-noisy-pyr-cosine} pyruvate-ammonium conditions; smaller distances indicate a greater similarity between the two conditions.
    Means of the set of Kendall's $\tau$-$b$ rank correlation coefficients between the parallel case and each biomass-prioritised case are shown \ref{fig:model-noisy-glc-kendall} for glucose-ammonium conditions and \ref{fig:model-noisy-pyr-kendall} pyruvate-ammonium conditions; larger means indicate a greater similarity between parallel biosynthesis and individual biomass component synthesis.
    }
