...
This expression means that the definition of the $r$ ratio is not trivial and depends on the $\lambda_{0}$ and the $\lambda_{i}^{\prime}$ values, which are independent of each other.
...
\begin{itemize}
  ...
  \item Growth rate varies linearly as $e_{\mathrm{pool}}$ varies.
        However, the ratio is not constant (but climbs up in steps).
        With higher $e_{\mathrm{pool}}$ values (less of an enzyme constraint), the ratio increases, indicating less advantage for sequential biosynthesis (what we want).
        Here, ratio < 1, but we've seen ratios > 1 in certain nutrient conditions.
  \item As $e_{\mathrm{pool}}$ increases, ablated flux all start out linear/linear-ish when the growth rate is still in the linear region.
...
\begin{itemize}
  \item Parsimonious FBA.
        This first uses FBA to compute optimal growth rate, fixes this value, then minimises sum of gene-associated reaction fluxes while maintaining optimal growth (either minimises the sum of fluxes or squared sum, depending on the package).
        Rejected because it fixes the growth rate, but we want to see how constraints affect cell strategies as they vary along a spectrum.
  ...
  \item Regularised FBA.
        \textcite{vijayakumarHybridFluxBalance2020} describe a quadratic program for solving a regularised bi-level FBA, i.e.\: $\max g^\intercal v - \frac{\sigma}{2}v^\intercal v$ where $g$ is the objective function, $v$ is the flux vector, and $\sigma$ is a regularisation parameter than can be tuned.
        Rejected because it requires a quadratic solver, thus difficult and computationally expensive to implement.
  ...
  \item Constrain the sum of absolute values of fluxes.
        Namely, $\sum |v| < c$, where $c$ is a constant to be varied.
        This makes sense for the Yeast8 (no enzyme constraint) model, and as $c$ decreases to 0, the original growth rate $\lambda_{0}$ and ablated growth rates $\lambda_{i}^{\prime}$ decreased to 0 linearly, while the $r$ ratio remains constant.  ADD PLOTS TO SHOW THIS?
\end{itemize}

These may have made sense for the Yeast8 (no enzyme constraint) model.
But if applied to ecYeast8,
there is potential double-/triple-counting of imposing constraints:
\begin{enumerate}
...
% TODO: Remove headers on images, in favour of putting them in captions
\begin{figure}
  \centering
...
% IDEA: Add barchart grids?  As a sanity check & it is related to the C:P ratio heatmap.
...
\begin{figure}
  ...
  \centering
    ...
    \caption{
      ...
      %Positive values (indigo) indicate pyruvate-limiting conditions, while negative values (beige) indicate ammonium-limiting conditions.
    }
...
More detail on how the quantities in the heatmaps are calculated...

% originally from caption, moved here to save space
% TODO: format this a bit, or organise into sensible bullet points.
...
Seems to divide the space into C-limited growth rate, N-limited growth rate, and neither-limited growth rate.
With extreme cases.
Red in ratio heatmap seems to correspond to the boundary between the C-limited and N-limited regions.
Growth rate near max, i.e.\ C/N not limiting: when glucose or ammonium exchange increases (there is an additive effect), temporal partitioning is more advantageous
N-limiting: as ammonium exchange increases, growth rate increases (decreasing $B$), and increasing $r$ implies $A$ increasing or staying the same.
...
The ratio > 1 part is around the boundary between C-limited growth rate and N-limited growth rate.
So it means that if both C and N are (nearly) limiting, sequential biosynthesis is so slow that the ratio goes above 1.
In the C-limited region, the C:P ratio is high (makes sense because when C is limiting, C takes long), and in the N-limited region, the C:P ratio is low (when N is limiting, P takes long).
