...
\begin{figure}
  ...
  \caption{
    ...
    Brightfield and fluorescent images are taken at regular intervals, and then processed using \textit{aliby} to provide time series of fluorescence intensity changes for each parent cell.
  }
...
It then uses \textit{BABY} \parencite{pietschLabelfreeMethodTrack2022} to segment the images of traps to identify the outlines of cells and to track cells from one time point to another, creating a lineage of cells.
\textit{aliby} then overlays the cell outlines onto the fluorescent (flavin and mCherry) images to extract fluorescence intensity, and assigns a fluorescence value to each cell at each time point based on the mean intensity of pixels within the cell's outline.
