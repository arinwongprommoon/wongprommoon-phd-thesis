...
% Are you sure is less well-characterised?
% Lloyd (2019) says it's the most studied one after the cell division cycle.
% But the Goldbeter citation below is 2022, so...
Because the glycolytic oscillation is less well-characterised, models that describe it are more coarse-grained, such as one driven by a single positive feedback loop \parencite{goldbeterMultisynchronizationOtherPatterns} [EXPLAIN THE MODEL A BIT MORE HERE]. % Also add more examples
...
% elaborate a bit more on these, i.e. what exactly did they do?

% Link to previous sections as appropriate.
% Expand/Elaborate citations as appropriate.
To extend on that precedent, there is further justification on the biological basis of flavins and flavoproteins.
They're linked to NAD(P)H via nitric oxide oxidoreductase (Yhb1) and NAD(P)H cycles have been implicated in bulk-culture \citep{tuLogicYeastMetabolic2005} % and more
...
% And discuss: 'flavin pool' concept.
...
KEGG pathway (see section ...) suggests that there is a mechanism for the cell to increase its flavin pool by uptake of riboflavin from the media or from biosynthesis; however, the experimenter can eliminate the former possibility by using minimal media that does not contain riboflavin.
...
% ------------------------------------------------------------------------------
% MOVE TO BIOLOGICAL RESULTS CHAPTER
...
% % (From 'Comments' section in org note)
% % Intended to come after review of at least the two studies I mentioned above.
% % This may lead to a change in the name of the sub-sub-section, or a small-scale reorganisation.
% \begin{itemize}
% \item Because budding yeast can uptake riboflavin via Mch5 (\cite{gudipatiFlavoproteomeYeastSaccharomyces2014}), using nutrient media that contains riboflavin may confound flavin oscillations.  Specifically, we cannot tell apart intracellular flavin that derives from riboflavin uptake and from \emph{de novo} biosynthesis.
% \begin{itemize}
% \item \emph{Evidence:} ramp experiment in SC shows increase in flavin signal while equivalent experiment in minimal (Delft) media does not (February 2021).
% \item Not 100\% with this logic yet.  Do we want to keep the 'flavin pool' constant? Because growing yeast in minimal media does not guarantee that the pool is constant; they can still make new flavin \emph{de novo}
% \end{itemize}
% \end{itemize}

% \begin{itemize}
% \item Judging by flavin measurements alone, we cannot be sure if changes are due to changes in the 'flavin pool' or due to changes in intracellular flavin redox state (as a function of intracellular redox state in general).
% \begin{itemize}
% \item I think a lot of the studies I've read assume a constant 'flavin pool'? And therefore the oscillations are therefore periodic shifts in redox equilibrium.
% \end{itemize}
% \end{itemize}
% ------------------------------------------------------------------------------
