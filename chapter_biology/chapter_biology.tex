...
\begin{itemize}
% Discussion: other things can reset phase.
...
% Discussion: this would also address chemostat/single-cell discrepancy.
\item Feast-and-famine, or glucose pulsing (\textcite{charvinForcedPeriodicExpression2009}).
\end{itemize}

% Need to explain why 7.5 g/L in the experiment that is showcased.
% Space to discuss contradictory results when cells start with 10 g/L glucose -- and hypothesise that if may be because the higher concentration allows greater stores, therefore retaining original behaviour for longer.
% I think the glucose-sensing papers I cited in intro touch upon this.
% Probably worth cracking open those experiments, showing the heatmap anyway, and see if the distribution/KS plots show us anything.
...
\textbf{Carbon source missing experiments:}
\begin{itemize}
...
\textbf{Deletion strain missing analysis:}
\begin{itemize}
...
\textbf{Deletion strain Missing experiments:}
\begin{itemize}
...
\end{itemize}
% Also including limiting glucose as it approximates chemostat conditions (\textcite{jonesCyberneticModelGrowth1999}, and add other citations that say that chemostat is starvation).  If chemostat conditions mean starvation, I should look at microfluidic conditions that mimic that best.
