...
And because the periodogram is produced from a analytical solution, you can compute the period and quantify the quality of the oscillation from this periodogram more easily than from a Fourier spectrum created from noisy data.
...
Figure~\ref{fig:analysis-ar} shows an example.
...
[INSERT MATHS OF THE STANDARD SCALER HERE]
...
\begin{enumerate}
  ...
  \item $C = 1$ [TODO: explain parameter]
  \item $\gamma = 1/N$, where $N$ is the number of time points.  [TODO: explain parameter]
\end{enumerate}

For featurisation, I explored using time points as features, the time series' Fourier spectrum as features, and using \texttt{catch22} features \parencite{lubbaCatch22CAnonicalTimeseries2019}.
...
Additionally, as a control, I ... [DESCRIBE WHAT `RANDOM' MEANS HERE].
...
The spectra [INSERT PLOT?] suggest that oscillatory cells exhibit a peak that is absent in non-oscillatory cells.
...
Feature vectors [INSERT PLOT?] seem to suggest that feature 10 (9 on the plots as arrays start from zero, \texttt{FC\_LocalSimple\_mean3\_stderr}, mean error from a rolling 3-sample mean forecasting) plays an important role in distinguishing the oscillatory and non-oscillatory time series. [EXPLAIN WTF IS FC\_Local...] [SOME FIGURES MAY BE USEFUL HERE.]
