...
\abstract{
  ...
  It entails oscillations in the concentrations and redox states of intracellular metabolites, oscillations in transcript levels, temporal partitioning of biosynthetic processes, and, in chemostats, oscillations in oxygen consumption.
...
%  The YMC has also been found to gate phases of the cell division cycle and to share regulatory mechanisms with other biological rhythms such as the circadian oscillator.
  ...
  it is unclear whether such cycles arise from interactions between cells or are generated independently by each cells in a culture.
%  Furthermore, the molecular mechanism of the YMC is unclear.
  ...
  Specifically, I use microfluidics to trap individual yeast cells and record the time-dependent intensity of flavin autofluorescence, a component of the YMC.

  Single-cell microfluidics produces a large amount of time series data.
  Noisy and short time series data produced from biological experiments restrict the computational tools that are useful for analysis.
  After considering the merits and caveats of such tools, I develop a method to filter time series, develop a simple machine learning model to classify whether time series are oscillatory, and develop a cross-correlation method to examine the periodicity of time series data.
...
%  These properties still hold when cells are cultivated in pyruvate, but the quality of oscillations is impaired in glucose-limiting conditions.
  I show that cells were able to individually reset the phase of their flavin oscillation, independently of the cell division cycle, in response to abrupt nutrient changes.
  I also show that deletion strains generated flavin oscillations that exhibited different behaviour from dissolved oxygen oscillations from chemostat conditions.
  ...
  My result show that in certain conditions, it is advantageous for the cell to sequentially synthesise biomass components as it shortens the timescale of biomass synthesis.

  This thesis thus confirms autonomous generation of flavin oscillations, and suggest a model in which the YMC responds to nutrient conditions and subsequently entrains the cell division cycle.
  It also emphasises the possibility that subpopulations in the culture explain chemostat-based observations of the YMC.
  Furthermore, this thesis paves the way for the use of computational methods for analysing large datasets of oscillatory time series, which is useful for many fields of study beyond the YMC.
}
